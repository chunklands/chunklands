 ...
 */
export function prng2(x: number, y: number, valueOffset = PRNG_OFF, valueMult = PRNG_MUL): number {
  return (1 +
          perlin2(valueOffset + x * valueMult, valueOffset + y * valueMult)) /
      2
}
 ...
 * @returns Normalized value [0; 1]
 */
export function prng3(x: number, y: number, z: number, seedOff = PRNG_OFF, seedMul = PRNG_MUL): number {
  return (1 +
          perlin3(
              seedOff + x * seedMul, seedOff + y * seedMul,
              seedOff + z * seedMul)) /
      2
}
