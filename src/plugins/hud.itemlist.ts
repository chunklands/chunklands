...
export default async function hudItemlistPlugin(
  registry: PluginRegistry
): Promise<HudItemlistPlugin> {
  ...
  const cleanup = createBatchCall()
    ...
    .add(
      engine.windowOn(window.handle, 'scroll', (event) => {
  ...
  function update() {
    const itemWidth =
      (contentSize.width - 2 * LEFT_PADDING - (ITEM_COUNT - 1) * ITEM_PADDING) /
      ITEM_COUNT;
    ...
    for (let i = 0; i < ITEM_COUNT; i++) {
      ...
      if (i === Math.round(selectedItem)) {
        engine.spriteInstanceUpdate(selection, {
          x,
          y,
          z: 0.15,
          show: true,
