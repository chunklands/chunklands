...
export default async function shutdownPlugin(registry: PluginRegistry, opts: IOpts) {

  const engine = await registry.get<EnginePlugin>('engine');
  const window = await registry.get<WindowPlugin>('window');

  const cleanup = createBatchCall().add(engine.windowOn(
      window.handle, 'shouldclose', () => {
