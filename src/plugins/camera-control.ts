...
export default async function pluginCameraControl(
  registry: PluginRegistry
): Promise<CameraControlPlugin> {
  ...
  const cleanup = createBatchCall()
    .add(
      engine.windowOn(window.handle, 'click', () => {
        engine.cameraAttachWindow(window.handle);
      })
    )
    .add(
      engine.windowOn(window.handle, 'key', (event) => {
        // ESC release
