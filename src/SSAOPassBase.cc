...
namespace chunklands {
  ...
  void SSAOPassBase::InitializeProgram() {
    ...
    *js_Program >> uniforms_.proj >> position >> normal >> noise;
    ...
    for (int i = 0; i < 64; i++) {
      ...
      GLUniform sample_uniform{"u_samples", i};
      *js_Program >> sample_uniform;
      sample_uniform.Update(sample);
    }
  }

  void SSAOPassBase::UpdateBufferSize(int width, int height) {
    DeleteBuffers();
  ...
  void SSAOPassBase::DeleteBuffers() {
