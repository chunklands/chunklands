#ifndef __CHUNKLANDS_COLLISION_HXX__
#define __CHUNKLANDS_COLLISION_HXX__
...
namespace chunklands::collision {
  ...
  struct collision_1D {
    ...
    explicit collision_1D() : start(-1.f), end(-2.f) {}
    explicit collision_1D(float start, float end) : start(start), end(end) {}
  ...
  struct collision_impulse {
    ...
    explicit collision_impulse(collision_1D collision) : collision(std::move(collision)) {}
